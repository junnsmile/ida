...
fypoints = pd.read_csv(r'D:\work\data\ertm\japan\fy4\fy_position_azimuth.csv')
profile_path = r'D:\work\data\ertm\japan\profile'
srf_path = r'D:\work\data\ertm\japan\srf'
t2mq2m_path = r'D:\work\data\ertm\japan\t2mq2m'
outpath = r'D:\work\data\ertm\japan\fync'
date = fypoints.iloc[:,4]
#print(Counter(np.array(date)))
...
for k in range(562,len(dates)):
    ...
    for lon in lons[0:3]:
        ...
        i = i+1
        print(str(k)+'******'+str(len(lons))+'*****'+str(file)+'******the*'+str(i)+'**round******')

    name = outpath+'\\era5_rt'+year+month+day+hour+'.nc'
    savencprofile(o3, h2o, pt, level,lons,list(latpro),sp,skt,name,u10,v10,t2m,sazs,saas)

    numbers = numbers+len(lons)
